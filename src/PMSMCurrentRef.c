...
void PMSMCurrentRefStep(const PMSMCurrentRef_Params_t *params,
  ...
  out->tqLim =
      (in->Vdc / params->Vnom) * fminf(params->Tmax, params->Pmax / speed);
  ...
  float I_nom =
      2.0f * (out->tqRefSat) / (3.0f * params->polePairs * params->fluxLink);
  if (M <= params->Modulation_Index_Threshold) {
    ...
    // Field weakening
    id = -params->Imax * (M - params->Modulation_Index_Threshold) /
